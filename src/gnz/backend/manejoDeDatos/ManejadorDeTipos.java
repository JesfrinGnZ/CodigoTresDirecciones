...
public class ManejadorDeTipos {
    ...
    /**
     * Se utiliza cuando se reconoce un dato numerico, y de esta forma asignar su tipo
     * @param s
     * @return 
     */
    ...
    public Dato buscarDatoMayor(DatoCodigo dato1, DatoCodigo dato2) {
        if (dato1 != null && dato2 != null) {
            if (dato1.getDato() == TipoDeDato.NUMERICO && dato2.getDato() == TipoDeDato.NUMERICO) {
                if (dato1.getDato().ordinal() > dato2.getDato().ordinal()) {
    ...
    /**
     * Crea los cuartetos,los datos ya deben tener el mismo tipo,
     * de haber incongruencias se devolvera null, o algun dato ya sera null si hubo problemas en
            ...
            switch (dato1.getDato()) {
                case CADENA:
                    if (operacion == Operacion.MAS) {
                        ...
                        ManejadorDeErrores.mostrarErrorSemantico(editor.getErroresTextArea(), mensaje, dato1.getNumeroDeLinea(), dato1.getNumeroDeColumna());
                        this.manCuarteto.setExistioErrorSemantico(true);
                    }
                    break;
                case BOOLEANO:
